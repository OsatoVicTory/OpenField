...
export default function InvestPage() {
    ...
    const categories = ["Agriculture", "Real Estate Mgt.", "Digital Arts", "Start ups", "Music"];
    ...
    const filterClick = useCallback((type: string, cIdx: number) => {
        ...
        setFilters((prev) => ({ ...prev, [type]: cIdx }));
    }, [tokens.length, state.loading]);
                ...
                <div className="w-full h-full overflow-y-auto">
                    ...
                    <div className="w-full border-b border-b-[rgb(38,39,45)] px-4 py-6 flex flex-col gap-y-6">
                        ...
                        <div className="flex flex-wrap w-full gap-x-3 gap-y-4">
                            ...
                            className={`w-fit text-[13px] rounded-md border border-[var(--border)] py-[4px] px-3 ${!state.loading && "cursor-pointer"} ${filters.cap === 0 ? "bg-[rgba(255,255,255,0.12)] font-medium text-white" : "text-white/70 bg-[var(--secondary3)]/20 hover:bg-[rgba(255,255,255,0.12)]"}`}>
                                {"All"}
                            </button>
                            {marketCaps.map((_cap, _cIdx) => (
                                <button onClick={() => filterClick("cap", _cIdx+1)} key={`filter_category_${_cIdx}`}
                                className={`w-fit font-medium text-[13px] rounded-md border border-[var(--border)] py-[4px] px-2 ${!state.loading && "cursor-pointer"} ${filters.cap === _cIdx+1 ? "bg-[rgba(255,255,255,0.12)] text-white" : "text-white/60 bg-[var(--secondary3)]/20 hover:bg-[rgba(255,255,255,0.12)]"}`}>
                                    {_cap}
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="w-full border-b border-b-[rgb(38,39,45)] px-4 py-6 mt-0 flex flex-col gap-y-6">
                        ...
                        <div className="flex flex-wrap w-full gap-x-3 gap-y-4">
                            ...
                            className={`w-fit text-[13px] rounded-md border border-[var(--border)] py-[4px] px-3 ${!state.loading && "cursor-pointer"} ${filters.cat === 0 ? "bg-[rgba(255,255,255,0.12)] font-medium text-white" : "text-white/70 bg-[var(--secondary3)]/20 hover:bg-[rgba(255,255,255,0.12)]"}`}>
                                {"All"}
                            </button>
                            {categories.map((_cat, _catIdx) => (
                                <button onClick={() => filterClick("cat", _catIdx+1)} key={`filter_category_${_catIdx}`}
                                className={`w-fit font-medium text-[13px] rounded-md border border-[var(--border)] py-[4px] px-2 ${!state.loading && "cursor-pointer"} ${filters.cat === _catIdx+1 ? "bg-[rgba(255,255,255,0.12)] text-white" : "text-white/60 bg-[var(--secondary3)]/20 hover:bg-[rgba(255,255,255,0.12)]"}`}>
                                    {_cat}
                                </button>
                            ))}
                        </div>
                ...
                <div className="w-full flex flex-col sticky top-[68px] right-0 z-11 bg-inherit">
                    <div className="w-full px-6 py-4 border-b border-b-[rgb(38,39,45)] flex items-center justify-between">
                        <div className="flex items-center w-fit gap-x-4">
                            ...
                            <div className="flex items-center w-fit gap-x-3 max-[400px]:hidden">
                                <button className={`w-fit flex items-center py-2 px-3 rounded-md border border-[var(--border)] ${!state.loading && "cursor-pointer"} ${topFilter === "Trending" ? "bg-[rgba(255,255,255,0.12)] text-white font-medium" : "bg-[var(--secondary3)]/30 hover:bg-[rgba(255,255,255,0.12)] text-white/60"}`}
                                onClick={() => !state.loading && setTopFilter("Trending")}>
                                ...
                                <button className={`w-fit flex items-center py-2 px-3 rounded-md border border-[var(--border)] ${!state.loading && "cursor-pointer"} ${topFilter === "New" ? "bg-[rgba(255,255,255,0.12)] text-white font-medium" : "bg-[var(--secondary3)]/30 hover:bg-[rgba(255,255,255,0.12)] text-white/60"}`}
                                onClick={() => !state.loading && setTopFilter("New")}>
                                ...
                                <button className={`w-fit flex items-center py-2 px-3 rounded-md border border-[var(--border)] ${!state.loading && "cursor-pointer"} ${topFilter === "Top" ? "bg-[rgba(255,255,255,0.12)] text-white font-medium" : "bg-[var(--secondary3)]/30 hover:bg-[rgba(255,255,255,0.12)] text-white/60"}`}
                                onClick={() => !state.loading && setTopFilter("Top")}>
                            ...
                            <div className="items-center w-fit gap-x-3 hidden max-[400px]:flex relative">
                                <button className={`w-fit flex items-center pt-1 pb-[6px] px-2 rounded-md border border-[var(--border)] ${!state.loading && "cursor-pointer"} bg-[rgba(255,255,255,0.12)] text-white font-medium`}
                        ...
                        <div className="w-fit flex items-center">
                            <button className={`bg-[var(--primary)] rounded-sm sm:rounded-full ${!state.loading && "cursor-pointer"} py-2 px-3 pr-4 flex items-center`}>
                    ...
                    <div className="Tokens_Table w-full overflow-x-auto hide_scrollbar" ref={scrollDrivenRef}>
                        <div className="w-fit min-w-full xl:w-full">
                            <header className={`w-full flex justify-between token_list_div items-center font-medium text-white/60 pl-2 pr-6 sticky top-0 right-0 z-9 py-1 border-b border-b-[var(--border)] `}>
                        ...
                        <div className="w-full CT_Ul">
                            {
                                ...
                                tokensLoadingArr.map((_, _lIdx) => (
                                    ...
                                    className={`w-full flex justify-between token_list_div font-medium items-center pl-2 pr-6 py-3 border-b border-b-[var(--border)] `}>
                                ...
                                (
                                    ...
                                    tokens.map((token, index: number) => (
                                        ...
                                        className={`w-full token_list_div flex justify-between font-medium items-center pl-2 pr-6 py-3 border-b border-b-[var(--border)] `}>
