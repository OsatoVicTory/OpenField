...
import { fetchAgentChats, sendAgentChat } from "@/app/actions/p2p";
...
export default function P2PChat({ vendor }: { vendor: P2PAgents }) {
    ...
    const FetchChats = useCallback(async () => {
        try {
            ...
            const res = await fetchAgentChats(vendor._id, user._id);
    ...
    const SendChat = useCallback(async () => {
        try {
            setSendLoading(true);
            const messageText: string = inputRef.current?.value || "";
            const message = { sender: "You", text: messageText, time: Date.now(), img: "" };
            const res = await sendAgentChat(vendor._id, user._id, message);
            ...
            <main className="flex-1 w-full z-1 overflow-y-auto px-3 lg:px-8 py-4">

                {
                    ...
                    <div className="w-full flex justify-center items-center py-5 px-4">
                        <button className="w-fit p-3 hover:bg-[var(--hover-bg)] cursor-pointer text-white/90"
                        onClick={FetchChats}>Refresh</button>
                    </div>
                    :
                    (
                        state.loading 
                        ?
                        <div className="w-full flex justify-center items-center py-5 px-4">
                            <Spinner className="w-[33px] h-[33px]" />
                        </div>
                        :
                            ...
                            <div className="flex w-full flex-col gap-y-2 text-xs py-1">
                                <div className="flex items-center gap-x-1">
                                    <span className="text-white/50">
                                        {!initialized ? `Limits: ${vendor.lowerLimits} - ${vendor.upperLimits} AGR` : `Bank / Chain: ${acct.bank_name}`}
                                    </span>
                                </div>
                                {initialized && <div className="flex items-center gap-x-1">
                                    <span className="text-white/50">Account Name: {acct.account_name}</span>
                                </div>}
                                
                                <div className="flex items-center gap-x-1">
                                    <span className="text-white/50">{!initialized ? `Balance: 10.000 AGR` : `Number/Address: ${acct.account_number}`}</span>
                                    {initialized && <>
                                        {!copied 
                                            ? 
                        ...
                        {!initialized && <div className="w-full flex flex-col justify-center bg-[#282828]/20 border border-[var(--border)] rounded-xl">
                            <div className="flex justify-between items-center border-b border-b-[rgb(38,39,45)] p-3">
                                <div className="w-fit flex items-center gap-x-1">
                                    ...
                                    <span className="text-xs text-white">{`${acct.bank_name} - ${acct.account_number}`}</span>
                                </div>
                                    ...
                                    {accts.map((_acct, a_idx) => (
                                        <div className="flex items-center p-2 justify-between w-full border-b border-b-[var(--border)] last:border-b-0" key={`acct-${a_idx}`}>
                                            <div className="text-inherit w-[calc(100%-50px)] truncate">
                                                {`${_acct.bank_name} - ${_acct.account_number}`}
                                            </div>
                                            <span className="text-white/50">|</span>
            ...
            <footer className="w-full p-3">
                <div className="flex flex-row w-full items-center gap-x-3 rounded-full justify-between p-2 bg-[#242626] shadow-[var(--message-footer-card)]">
                    ...
                    <div className="grow-1 flex items-center h-fit">
                        <textarea id="type-textarea" ref={inputRef} className="w-full text-sm field-sizing-content h-fit resize-none border-none outline-none bg-transparent min-h-[23px] max-h-[100px]" placeholder="Type a message"></textarea>
                    </div>
                    <button onClick={SendChat}
                    className={`p-2 flex items-center justify-center rounded-full bg-[var(--primary-light)] ${!sendLoading&&"cursor-pointer"}`}>
                        {
                            sendLoading 
                            ?
